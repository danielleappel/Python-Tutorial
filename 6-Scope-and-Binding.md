...
## Assignment
As discussed in [5-Functions-and-Parameters.md](https://github.com/danielleappel/Python-Tutorial/blob/main/5-Functions-and-Parameters.md), Python passes mutable objects (like lists) by reference and immutable objects (like ints, strings, and tuples) by value. The difference between the two can be seen in two similar examples: one with a list, and one with a tuple.
